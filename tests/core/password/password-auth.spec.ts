...
describe('core/password/password-auth', () => {
  it('registers a user and creates a session', async () => {
    ...
    const res = await core.registerPassword({
  ...
  it('does not enumerate: wrong password and unknown user both fail with password_invalid', async () => {
    ...
    await expect(
      core.loginPassword({ identifier: 'a@example.com', password: 'wrong-password-here' })
    ).rejects.toMatchObject({
      code: 'password_invalid'
    });
    await expect(
      core.loginPassword({ identifier: 'missing@example.com', password: 'wrong-password-here' })
    ).rejects.toMatchObject({
      code: 'password_invalid'
    });
  });
  ...
  it('rehashes (upgrades) when desired params are stronger', async () => {
    const mem = makeMemoryStorage();
    const coreWeak = createAuthCore({
      storage: mem.storage,
      passwordHashParams: { memoryCost: 8192 }
    });
    await coreWeak.registerPassword({
      identifier: 'a@example.com',
      password: 'long-enough-password'
    });
    ...
    const coreStrong = createAuthCore({
      storage: mem.storage,
      passwordHashParams: { memoryCost: 16384 }
    });
    await coreStrong.loginPassword({
      identifier: 'a@example.com',
      password: 'long-enough-password'
    });
