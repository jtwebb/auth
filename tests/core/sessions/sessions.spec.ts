...
describe("core/sessions", () => {
  it("validates an existing session and touches lastSeenAt", async () => {
    ...
    const res = await createAuthCore({ storage: mem.storage, clock: { now: () => later } }).validateSession({
  ...
  it("revokes a session token", async () => {
    const mem = makeMemoryStorage();
    const now = new Date();
    const core = createAuthCore({ storage: mem.storage, clock: { now: () => now }, randomBytes: (n) => new Uint8Array(n).fill(1) });
