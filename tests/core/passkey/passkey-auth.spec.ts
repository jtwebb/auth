...
import type { AuthStorage, SessionRecord, StoredChallenge, WebAuthnCredentialRecord } from "../../../src/core/storage/auth-storage.js";
...
function makeMemoryStorage() {
  ...
  const storage: AuthStorage = {
    ...
    webauthn: {
      listCredentialsForUser: async (userId) => [...credentials.values()].filter((c) => c.userId === userId),
...
describe("core/passkey/passkey-auth", () => {
  ...
  it("finishPasskeyRegistration persists a credential", async () => {
    ...
    const start = await core.startPasskeyRegistration({ userId: "u1" as any, userName: "a@example.com" });
  ...
  it("finishPasskeyLogin creates a session and updates the counter", async () => {
    ...
    const finish = await core.finishPasskeyLogin({ challengeId: start.challengeId, response: { id: "cred-1" } as any });
