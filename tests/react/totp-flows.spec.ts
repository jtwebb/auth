...
describe("react/totp-flows", () => {
  it("startEnrollment and finishEnrollment call JSON endpoints", async () => {
    const fetchFn = vi.fn(async (url: any, init: any) => {
      if (String(url).endsWith("/totp/start")) {
        ...
        return new Response(JSON.stringify({ userId: "u1", secretBase32: "ABC", otpauthUri: "otpauth://totp/x" }), {
          status: 200,
        });
      }
    ...
    const flows = createTotpFlows(
      { enrollmentStartUrl: "/totp/start", enrollmentFinishUrl: "/totp/finish", verifyUrl: "/totp/verify" },
      { fetchFn },
    );
  ...
  it("verify posts form body", async () => {
    const fetchFn = vi.fn(async (_url: any, init: any) => {
      expect(String(init.headers["content-type"] ?? init.headers.get?.("content-type"))).toContain("application/x-www-form-urlencoded");
      expect(String(init.body)).toContain("code=123456");
      return new Response("", { status: 200 });
    });

    const flows = createTotpFlows({ enrollmentStartUrl: "", enrollmentFinishUrl: "", verifyUrl: "/totp/verify" }, { fetchFn });
